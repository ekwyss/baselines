...
class DDPG(object):
    ...
    #             print("INIT DEMO BUFFER SAMPLE CALLED")
    #             transitions = self.sample_transitions(episode, num_normalizing_transitions)#, [episode['u'].shape[1]]*episode['u'].shape[0])
    #             print("INIT DEMO BUFFER SAMPLE OUT")
    ...
    # def store_episode(self, episode_batch, sg_ts, num_sg_ts, update_stats=True):
    #     """
    ...
    #         print("STORE EPISODE SAMPLE CALLED")
    #         transitions = self.sample_transitions(episode_batch, num_normalizing_transitions)#, ep_Ts)
    #         print("STORE EPISODE SAMPLE OUT")
    ...
    def update_stats(self, episode_batch, ts, num_sg_ts, cand_eps):
        #check if any cand_eps exist
        ...
        episode_batch['ag_2'] = episode_batch['ag'][:, 1:, :]
        # num_normalizing_transitions = transitions_in_episode_batch(episode_batch)
        transitions = self.sample_transitions(episode_batch, num_sg_ts, ts, cand_eps)#, ep_Ts)
    ...
    def _grads(self):
        # self.sess.run(self.mask, feed_dict={self.mask:np.concatenate((np.zeros(self.batch_size - self.demo_batch_size), np.ones(self.demo_batch_size)), axis = 0)})
        # print(self.batch_size, self.demo_batch_size)
        # self.sess.run(self.mask, feed_dict={self.mask_zeros:self.batch_size,self.mask_ones:self.demo_batch_size})
        # print(self.mask)
        # self.sess.run(self.mask, feed_dict={self.mask:np.concatenate((np.zeros(self.batch_size - self.demo_batch_size), np.ones(self.demo_batch_size)), axis = 0)})
        ...
        critic_loss, actor_loss, Q_grad, pi_grad = self.sess.run([
            ...
            self.Q_grad_tf,
            self.pi_grad_tf],
            feed_dict={self.mask:np.concatenate((np.zeros(self.batch_size), np.ones(self.demo_batch_size)), axis = 0)}
        )
    ...
    # def sample_batch(self, batch_size, demo_batch_size):
    def _preprocess_batch(self, transitions):
    ...
    def _create_network(self, reuse=False):
        ...
        # self.mask_zeros = tf.placeholder(tf.int32, shape=[])
        # self.mask_ones = tf.placeholder(tf.int32, shape=[])
        self.mask = tf.placeholder(tf.int32, shape=[None])
        # self.mask = tf.compat.v1.Variable(tf.concat([tf.zeros(self.mask_zeros),tf.ones(self.mask_ones)],axis=0), validate_shape=False)
        # self.mask = tf.compat.v1.Variable([], validate_shape=None)
        # self.mask.set_shape([None])

        # mask = tf.compat.v1.Variable(np.concatenate((np.zeros(self.batch_size - self.demo_batch_size), np.ones(self.demo_batch_size)), axis = 0))
        # mask = tf.placeholder(tf.int64, shape=(256,))
        ...
        if self.bc_loss ==1 and self.q_filter == 1 : # train with demonstrations and use bc_loss and q_filter both
            # maskMain = tf.reshape(tf.boolean_mask(self.main.Q_tf > self.main.Q_pi_tf, self.mask), [-1]) #where is the demonstrator action better than actor action according to the critic? choose those samples only
            #define the cloning loss on the actor's actions only on the samples which adhere to the above masks
            self.cloning_loss_tf = tf.reduce_sum(tf.square(tf.boolean_mask(tf.boolean_mask((self.main.pi_tf), self.mask), tf.reshape(tf.boolean_mask(self.main.Q_tf > self.main.Q_pi_tf, self.mask), [-1]), axis=0) - tf.boolean_mask(tf.boolean_mask((batch_tf['u']), self.mask), tf.reshape(tf.boolean_mask(self.main.Q_tf > self.main.Q_pi_tf, self.mask), [-1]), axis=0)))
        ...
        tf.compat.v1.variables_initializer(self._global_vars('')).run(feed_dict={self.mask:np.concatenate((np.zeros(self.batch_size - self.demo_batch_size), np.ones(self.demo_batch_size)), axis = 0)})
        # tf.compat.v1.variables_initializer(self._global_vars('')).run(feed_dict={self.mask_zeros:self.batch_size, self.mask_ones:self.demo_batch_size})
