...
def make_sample_her_transitions(replay_strategy, replay_k, reward_fun, num_goals):#, policy_index):
    ...
    def _sample_her_transitions(episode_batch, batch_size_in_transitions, checkpoint_Ts):#, ep_Ts):
        ...
        transition_Ts = np.empty(len(her_indexes))
        # print(her_indexes)
        for i,her_index in enumerate(her_indexes):
            ep = episode_idxs[her_index]
            t = t_samples[her_index]
            transition_Ts[i] = checkpoint_Ts[episode_idxs[her_index]][np.searchsorted(t < checkpoint_Ts[episode_idxs[her_index]], True)] #np.where(t < checkpoint_Ts[episode_idxs[her_index]])[0][0]
            # print(episode_idxs[her_index], t_samples[her_index],checkpoint_T, checkpoint_Ts[episode_idxs[her_index]])

        # print(t_samples[her_indexes])
        # print(transition_Ts)
        future_offset = np.random.uniform(size=len(her_indexes)) * (transition_Ts - t_samples[her_indexes])
        future_offset = future_offset.astype(int)
        # print(future_offset)
        future_t = (t_samples[her_indexes] + 1 + future_offset)
        # print(future_t)
        ...
        # #Currently calculating future offsets for entire batch size for each episode when only using len(her_idxs) many
        # #instead of storing Ts_IJ and replacing future_P% of goals every time, store future_t themselves

        # #TODO: Also see if explicitly enforcing "final" changes much
        # for i in range(rollout_batch_size):
        #     ######This is for proportional sampling################
        ...
        #     ##############This is for within subgoal sampling#####################
        #     for j in range(3):
        #         sg_T = sg_Ts[j]
        ...
        # # print(t_samples)
        # # print(future_p) #0.8
        # print(len(future_offsets),len(t_samples),rollout_batch_size, batch_size, t_per_ep)
...
#     def _sample_her_transitions(episode_batch, batch_size_in_transitions, Ts_IJ):#, ep_Ts):
...
#         # print("Ts: ",Ts_IJ, rollout_batch_size, 3)
#         # print("stored Ts:",stored_Ts_IJ)
